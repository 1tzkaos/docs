...
- `X-RateLimit-Limit` — total cap for the current 60-second window (= `rps_cap × 60`).
...
| 429 | `RATE_LIMIT_EXCEEDED` | RPS cap exceeded in current window. Retry after `X-RateLimit-Reset` seconds. |
