...
- `/tokens/{mint}/intel` and `/tokens/{mint}/risk` also return `error` as a **string**, for their 400 (malformed `mint`), 404 (unknown mint) and 503 (the lookup failed; retry): `{ "success": false, "data": null, "error": "not found: mint not found: <mint>" }`. A failed lookup is never reported as a 404; `/tokens/{mint}/risk` answers 504 if its lookup times out.
