...
| `unavailable_signals` | Meaning | What to do |
|---|---|---|
| `[]` | Every read succeeded. Anything in `missing_signals` is data that does not exist yet. | Use the response. |
| non-empty | Those signals could not be read this time. They are not scored as passed. | Retry. The response is sent `Cache-Control: no-store`, so a retry is not served from a cache. |

The risk block embedded in `/tokens/{mint}/intel` carries the same `status`, `missing_signals` and `unavailable_signals`.
