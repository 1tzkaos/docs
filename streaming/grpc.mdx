...
Each `Subscribe` stream counts against your key's concurrent-stream allowance, which it shares with `SwapStream` and the `/ws` WebSocket: Free 1, Developer 10, Pro 50, Enterprise unlimited. One more stream past the cap returns `RESOURCE_EXHAUSTED`. See [Streaming caps](/concepts/quotas-errors#streaming-caps).
