...
asyncapi: "/api-reference/asyncapi.json pumpLifecycle"
---
...
For one-shot history of the same events, use the REST endpoint `GET /pump-lifecycle` (under **Pump lifecycle** in the API reference). That endpoint takes a `status` filter — `new`, `completing`, `graduating`, or `migrated` — and returns **400** `invalid_filter:status:<value>` for anything else.
