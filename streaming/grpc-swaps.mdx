...
The cap is one allowance per key, shared with `PriceStream` and the `/ws` WebSocket (see [Streaming caps](/concepts/quotas-errors#streaming-caps)). Exceeding it returns `RESOURCE_EXHAUSTED` with a message like `stream cap exceeded; tier=developer cap=10`.
