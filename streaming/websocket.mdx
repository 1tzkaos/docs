...
- **Nullable fields.** `virtual_*_reserves` are non-null only on pump.fun. `fee`, `creator_fee`, `price_impact_bps`, `cashback` come back null on protocols that don't report them — fall back to the `_bps` siblings. `price_impact_bps` is only reported by `pumpfun` and `pumpswap`; where a venue doesn't report it you get `null` here (REST returns `0`), which means *not reported*, not zero impact.
