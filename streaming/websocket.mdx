...
The same socket also pushes `pool_event` frames whenever an LP operation matches your filter. Frame shape mirrors [`GET /pool-events`](/api-reference/pool-events/listpoolevents) with a `type` discriminator on top:
